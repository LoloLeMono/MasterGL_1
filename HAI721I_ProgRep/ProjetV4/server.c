...
int main(int argc, char *argv[])
{
    ...
    while (compt < nbNodes)
    {
        ...
        printf("SERVER : Je regarde les voisins de %d \n", compt);
        
        for(int i=0; i<nbNodes; i++)
        {
            printf("SERVER : %d à t-il un lien avec %d \n", compt, i);

            if (tabNodes[compt][i] == 1)
            {
                /* Etape 5.3.1 : Envoyer l'adresse */
                /* printf("SERVER : J'envoi les adresses de chaques voisins \n"); */

                /* Etape 5.3.2 : Envoyer la socket */
                printf("SERVER : J'envoi la socket de %d à %d \n", compt, i);
                snd = sendto(ds, &tabSock[i], sizeof(struct sockaddr_in), 0, (struct sockaddr*) &tabSock[compt], sizeof(struct sockaddr_in));
            }
        }
